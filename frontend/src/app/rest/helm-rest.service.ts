...
import {ChangeRegistryPasswordDTO} from "../registries/change-registry-password-dto";
...
export class HelmRestService {
  ...
  public persistHelmRegistry(helmRegistry: HelmRegistry): Observable<HelmRegistry> {
    if (helmRegistry.isNew()) {
      //create new helmRegistry call
      return this.http.post<HelmRegistryDTO>(this.root_path, helmRegistry).pipe(map(helmRegistryDTO => HelmRegistry.from(helmRegistryDTO)));
    } else {
  ...
  public changeHelmRegistryPassword(helmRegistry: HelmRegistry, passwordDTO: ChangeRegistryPasswordDTO): Observable<HelmRegistry> {
