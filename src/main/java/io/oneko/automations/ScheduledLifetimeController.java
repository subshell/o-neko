...
public class ScheduledLifetimeController {
	...
	public ScheduledLifetimeController(LifetimeBehaviourService lifetimeBehaviourService,
		...
		this.scheduledProjectCheckTimer = Timer.builder(new MetricNameBuilder().durationOf("lifetime.scheduled.checkProjects")
						.build())
		...
		this.retrieveExpiredDeploymentsTimer = Timer.builder(new MetricNameBuilder().durationOf("lifetime.scheduled.deployments.retrieveExpired")
						.build())
				.description("the time it takes O-Neko to filter and retrieve expired deployments")
				.publishPercentileHistogram()
				.register(meterRegistry);
		this.expiredDeploymentStopTimer = Timer.builder(new MetricNameBuilder().durationOf("lifetime.scheduled.deployments.stopExpired")
						.build())
	...
	public void checkProjects() {
		...
		final List<ProjectVersion<?, ?>> versions = projectRepository.getAll()
				.stream()
				.map(ReadableProject::writable)
				.flatMap(project -> project.getVersions()
						.stream())
	...
	private void stopExpiredDeployments(List<ProjectVersion<?, ?>> deployables, Consumer<ProjectVersion<?, ?>> beforeStopDeployment) {
		...
		expiredPairsOfDeployableAndDeployment.forEach(expiredVersionDeploymentPair -> expiredDeploymentStopTimer.record(() -> {
	...
	private boolean shouldConsider(Optional<LifetimeBehaviour> behaviour) {
		return behaviour.isPresent() && !behaviour.get()
				.isInfinite();
	}

	private List<Deployment> getRelevantDeploymentsFor(List<ProjectVersion<?, ?>> deployables) {
		final var uuids = deployables.stream()
				.map(ProjectVersion::getId)
				.collect(Collectors.toSet());
		return deploymentRepository.findAllByProjectVersionIdIn(uuids)
				.stream()
				.filter(deployment -> !deployment.getStatus()
						.equals(DeployableStatus.NotScheduled))
				.collect(Collectors.toList());
	}

	private Set<Pair<ProjectVersion<?, ?>, Deployment>> getExpiredPairsOfDeployableAndDeployment(List<ProjectVersion<?, ?>> versions,
	...
	//what a method name
	private Function<Deployment, Optional<Pair<ProjectVersion<?, ?>, Deployment>>> createExpiredDeployableDeploymentCombiningFunction(
			List<ProjectVersion<?, ?>> deployables) {
		return (deployment) -> {

			final var matchingDeployableOptional = deployables.stream()
					.filter(d -> d.getId()
							.equals(deployment.getProjectVersionId()))
					.findFirst();
