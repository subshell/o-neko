...
class DockerRegistryPolling {
	...
	private void startPollingJob() {
		...
		final Mono<List<WritableProjectMesh>> meshes = projectMeshRepository.getAll().map(ReadableProjectMesh::writable).collectList();
		final Mono<List<ReadableProject>> projects = projectRepository.getAll().collectList();
	 ...
	 */
	private Mono<Void> checkDockerForNewImages(List<WritableProjectMesh> allMeshes, List<ReadableProject> allProjects) {
		Flux<WritableProject> updatedProjectVersions = Flux.fromIterable(allProjects)
	...
	private Mono<WritableProject> fetchMissingDatesForImages(WritableProject project) {
		final List<WritableProjectVersion> versionsWithoutDate = project.getVersions().stream()
				...
				.filter(version -> !failedManifestRequests.contains(version.getId()))
				...
				.flatMap(version -> dockerRegistryV2ClientFactory.getDockerRegistryClient(project)
						...
						.map(manifest -> {
							if (manifest.getImageUpdatedDate().isPresent()) {
								...
								failedManifestRequests.add(version.getId());
							}
							return version;
						}))
				.collectList()
	...
	private Mono<ReadableProject> updateProjectVersions(ReadableProject project) {
		...
		return projectDockerClient
				...
				.flatMap(tags -> {
					WritableProject writable = project.writable();
					ImmutableList<ReadableProjectVersion> versions = project.getVersions();
					...
					newVersions.forEach(version -> {
						WritableProjectVersion projectVersion = writable.createVersion(version);
						resultingEvents.add(new NewProjectVersionFoundEvent(projectVersion, this.asTrigger));
					});

					removedVersions.forEach(version -> {
						WritableProjectVersion projectVersion = writable.removeVersion(version);
					...
					if (newVersions.size() > 0 || removedVersions.size() > 0) {
						Mono<ReadableProject> projectMono = projectRepository.add(writable)
								.subscriberContext(Context.of(EventTrigger.class, this.asTrigger));
	...
	private Mono<?> redeployAllByManifest(Manifest manifest, WritableProjectVersion version, Collection<WritableMeshComponent> components) {
		...
		if (!StringUtils.equals(version.getDockerContentDigest(), digest)) {
			...
			version.setImageUpdatedDate(manifest.getImageUpdatedDate().orElse(null));
			dependingMonos.add(this.redeployAndSaveVersion(version).switchIfEmpty(Mono.empty()));
		}
		return Flux.concat(dependingMonos).collectList();
	}

	private Mono<ReadableProjectVersion> redeployAndSaveVersion(WritableProjectVersion version) {
